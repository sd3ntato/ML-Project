...
class MLP():
  ...
  def __init__(self, Nh=16, Nl=2, Nu=1, Ny=1, f=tanh, f_out=ide ,w_scale=.05, loss=squared_error):
    ...
    self.v = [None]*(Nl+2) # potenziali attivazione
    self.x = [None]*(Nl+2) # attivazioni
    self.d = [None]*(Nl+2) # coefficenti di propagazione
    self.grad = [None]*(Nl+1) # gradienti
    self.l = loss # funzione loss
    self.dl = derivative(loss)
    ...
    self.w[0] = (2*np.random.rand(Nh,Nu+1)-1)*w_scale # pesi input-to-primo-layer, ultima colonna e' bias
    for i in range(1,Nl):
  ...
  def backward_pass(self, y ): # calcolo coefficenti di propagazione errore
    ...
    self.d[Nl+1] = self.dl( y , self.x[Nl+1]) * self.df[Nl+1](self.v[Nl+1]) 
